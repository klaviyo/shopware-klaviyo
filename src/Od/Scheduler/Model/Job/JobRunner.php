...
use Klaviyo\Integration\Od\Scheduler\Async\{JobMessageInterface, ParentAwareMessageInterface};
use Klaviyo\Integration\Od\Scheduler\Entity\Job\JobEntity;
use Klaviyo\Integration\Od\Scheduler\Model\Exception\JobException;
    ...
    private function postProcessResult(
        ...
        if ($message instanceof ParentAwareMessageInterface) {
            ...
            if ($this->jobHelper->getChildJobs($parentJobId, self::NOT_FINISHED_STATUSES)->count() === 0) {
                $hasFailedChild = $this->jobHelper->getChildJobs($parentJobId, [JobEntity::TYPE_FAILED, JobEntity::TYPE_PARTIALLY])->count() !== 0;
                /**
